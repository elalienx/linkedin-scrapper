...
export default async function scrapLinkedIn(url) {
  ...
  const jobTitle = await page.$eval(jobTitleTag, (item) => item.textContent.trim());
  const pic = await page.$eval(picTag, (item) => item.src);
  const jobCompany = await page.$eval(jobCompanyTag, (item) => item.textContent.trim());
  const jobDateRange = await page.$eval(jobDurationTag, (item) => item.textContent.trim());
